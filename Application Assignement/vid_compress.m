function [all_frames,rmse_vec] = vid_compress(t_l)
    [Eu, Ct_mat, T,sigma, original_frames] = compute_coded_snapshot_with_noise(t_l);
    fname_coded = sprintf('output_f/coded_frames/frames_%d_to_%d.png',t_l,t_l+T-1);
    imwrite(mat2gray(Eu),fname_coded,'PNG');
    [H,W] = size(Eu);
...
%         rmse_vec(i) = rmse(all_frames(:,:,i),original_frames(:,:,i));
        ...
        i1 = (t_l - 1) + i;
        fname = sprintf('output/frame_%d.png',i1);
        imwrite(uint8(save_img),fname,'PNG');
        
        fname_2 = sprintf('output_f/reconstructed/frame_%d.png',i1);
        imwrite(uint8(save_img(:,1:W)),fname_2,'PNG');
        
        fname_3 = sprintf('output_f/original/frame_%d.png',i1);
        imwrite(uint8(save_img(:,W+1:end)),fname_3,'PNG');
